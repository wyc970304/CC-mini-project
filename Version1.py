...
def RAM_database():
        ...
        return jsonify({'Welcome':'Basic character information of RickandMothy'}), 200
        # If success, return welcome and 200
...
def create_character():
        ...
        if len(result):
                ...
                session.execute("""INSERT INTO REM.character(id, name, gender, status.species) \ VALUES({}, '{}', '{}', '{}', '{}')""".format(int(request.json['id']), request.json['name'], request.json['gender'], request.json['status'], request.json['species']))
